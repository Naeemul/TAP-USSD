    ...
    if ($state =='node') {
        ...
        if (mysqli_num_rows($result3) > 0) {
            while ($row3 = mysqli_fetch_assoc($result3)) {
                ...
                if ( ($row3['branch'] == 'TBL-USER' && $row3['state'] == 14142) || ($row3['branch'] == 'TBL-AGENT' && $row3['state'] == 16142) ) {
                    $ztype = 'INPUT';
                }
            }
        }
        ...
        $sql2 = "SELECT id, process, text, pervnode, type, userprocess FROM appmenu where nid = '".$ustate."' and service= '".$service."'";
        //echo "<br>".$sql2."<br>";
        ...
        if (mysqli_num_rows($result2) > 0) {
		    while ($row2 = mysqli_fetch_assoc($result2)) {
                ...
                // pin first array
                $pinFirstSkul = array("DCGPC","LPSC","BGBPS","DPSC","CESC");
                ...
                if (in_array($row2["id"], $flashMessageId)) {
                    ...
                    $message = $row2["process"]." BDT ".$input." toP2P ".$agentno."|Please enter PIN to confirm: <200";
                } 
                ...
                elseif ($row2["id"] == 42 ||$row2["id"] == 128) {
                   /* $DESCOresp = GetDESCOAmount($input);
                    ...
                    $DESCOresp = getDESCOPostPaidFetch($input, $ussdid);
                   
                   if ((float)$DESCOresp[0] > 0 && strlen($DESCOresp[1])>0 ) {
                       $message = "Pay Tk". $DESCOresp[0] ." to DESCO for Bill Number ". $input ."|Enter PIN to Confirm:";
                      
                       $sqldescopost = "INSERT INTO ussdinputs (msisdn, input) 
                       VALUES  ('".$ussdid."',  '".$DESCOresp[1]."')";
                        $conn->query($sqldescopost);
                    } 
                    else {
                       if (strlen($DESCOresp[0])>0){
                          $message =  $DESCOresp[0]."|Enter 0 to Retry:";
                        }else{
                            $message =  "Please try again|Enter 0 to Retry:";
                        }
                    }
                    // $message = $DESCOamount;
                } 
                ...
                elseif (in_array($row2["id"], [142, 56, 140, 54])) {
                    ...
                    elseif (
                        in_array(strtoupper($skulinputarry[0]), ['CCPC', 'SGC', 'SCBHS', 'BNSCK', 'SHCPSC', 'JCPSC', 'AFMC', 'SCBBHS', 'RGC', 'GCBHS', 'HCPSC', 'RCBS', 'CDC', 'NSTU', 'HSDYKM', 'LIAMAD', 'ARMC', 'AMC', 'MKBMC', 'FCC', 'TIS', 'NCM', 'SEPI', 'KPI', 'NASCD','CBHSR', 'DC','MIS','RVCBS','NIC','CPSCJK', 'CBHSG','RUMC','AFMC' ,'CPSCBUSMS', 'JPSC' , 'SSMHS', 'BISC' , 'DCGPC' , 'GNIBC', 'IPSC','BBCPSC','HCISC','GTSC','DRMC' ,'NSTUC', 'BGBPS' , 'MTBMC', 'BGTSC' , 'JCESC', 'SCPSC1' ,'AKI', 'PCSC', 'BHGTSC', 'CBHSM' ,'MGSC' ,'AAIKM', 'GPCPS' ,'BISCM' , 'FCS' , 'BGPSCR' ,'RCPSC1','GCPSC','TSN','CGHS' ,'RMC' ,'GCBGHS','LSCC','CBHSJ' ,'RCPSC2', 'CHS' ,'BGPSC' , 'IIT' , 'PPI' ,'DCBABN' ,'BPSC' ,'CPSCR','TMSR' ,'SSHSR','CBGSR','CBSMS' ,'SCPSC' ,'GES' ,'NPSNS1' ,'MCPSC','BCC' ,'NKCHS','BPSCC' , 'NCPSC', 'RCPSC' , 'NASCC', 'NPSNS2','ABMGSC', 'PISE' ,'BNSCC' ,'NASCK', 'NSTUBBH','CIM' ,'MMA','CPSCL','DPSC','CPSCS' ,'CPSCH' , 'UPIB' ,'BAFSK' ,'CPSCEVJES', 'BKSP', 'ClsXI' ,'PROYASHCTG', 'CCC','BSMRMU','NSTUE','CBBHSC' ,'CBMB','JESC','SAGC' ,'ACC' ,'CBSS' ]) &&
                        in_array($row2["id"], [140, 54])
                    ) {
                        // Cus]tom Month/Year prompt for CCPC
                ...
                /* Mobile Recharge second step show - 1. Prepaid 2. Postpaid */ 
                ...
                elseif ( $row2["id"] == 590 || $row2["id"] == 579){
                    $sql_input = "SELECT * FROM ussdinputs WHERE msisdn = '".$ussdid."' order by id";
                    $inputussd = array();
                                          
                    $result = mysqli_query($conn, $sql_input );
                    while ($row = mysqli_fetch_assoc($result)) {           
                        $ussdinput = $row['input'];
                        array_push($inputussd, $ussdinput);
                    }
                    if (isValidMSISDN($inputussd[3]) == false){
                        $message =  "Provided MSISDN is not valid. Please start with 01. Type 0 for initial menu.";
                    }
                    else {
                        $message = $row2["text"].'<200';
                    }
                ...
                elseif ($row2["id"] == 600 || $row2["id"] == 774) {
                   /* $mpay_sql = "SELECT GROUP_CONCAT(input_value SEPARATOR ' ') as userinput FROM mdnstate WHERE mdn = '".$ussdid."' AND state like '%S' GROUP BY mdn";
                    ...
                    //$message =  "You are paying Tk. ". $inputussd[2] ." to ".  $inputussd[1] ." for ".   $inputussd[3] ."|Enter PIN to confirm:";
                    //$message =  "You are paying Tk. ". $mpay_arry[1] ." to ". $mpay_arry[0] ." for ". $mpay_arry[2] ."|Enter PIN to confirm:";
                    $message =  "You are paying Tk. ". $inputussd[2] ." to ".  $inputussd[1] ." for ".   $inputussd[3] ."|Enter PIN to confirm:";
                } 
                else {
                    ...
                    if (mysqli_num_rows($get_user_input_result) > 0) {
                        while ($get_user_input_row = mysqli_fetch_assoc($get_user_input_result)) {
                            ...
                            elseif (strtoupper($inputarry[0]) == 'BUP') {
                                ...
                                if (strtolower($row2["userprocess"]) == 'desco' && $pre_post_data[2] == 1){

                                    /*echo "<pre>";
                                    print_r($pre_post_data);
                                    echo "</pre>";*/
                                    $output = getDESCOPostPaidPayment($pre_post_data,$ussdid);
                                    $output .= '<400';
                                    $output=  $output.'<400';
                                

                                }
                                else if (strtolower($row2["userprocess"]) == 'despr' && $pre_post_data[2] == 2) {
                                    ...
                                    $output = sendTopupNewProcessUSSD(getTopUpParamsUSSD($ussdid, $userinput), $userinput);
                                    $output .= '<400';
                                    $output=  $output.'<400';
                                }     
                                elseif (strtolower($row2["userprocess"]) == 'mpay') {
                                    ...
                                    $output = mpayService($ussdid, $inputussd);
                                    $output=  $output.'<400';
                                ...
                                elseif ($row2["userprocess"] == '' && preg_match('/^\s?(CCPC|SGC|SCBHS|BNSCK|SHCPSC|JCPSC|AFMC|SCBBHS|RGC|GCBHS|HCPSC|RCBS|NSTU|CDC|HSDYKM|LIAMAD|ARMC|AMC|MKBMC|FCC|TIS|NCM|SEPI|KPI|NASCD|CBHSR|DC|MIS|RVCBS|NIC|CPSCJK|CBHSG|RUMC|AFMC|CPSCBUSMS|JPSC|SSMHS|BISC|DCGPC|GNIBC|IPSC|BBCPSC|HCISC|GTSC|DRMC|NSTUC|BGBPS|MTBMC|BGTSC|JCESC|SCPSC1|AKI|PCSC|BHGTSC|CBHSM|MGSC|AAIKM|GPCPS|BISCM|FCS|BGPSCR|RCPSC1|GCPSC|TSN|CGHS|RMC|GCBGHS|LSCC|CBHSJ|RCPSC2|CHS|BISC|BGPSC|IIT|PPI|DCBABN|BPSC|CPSCR|TMSR|SSHSR|CBGSR|CBSMS|SCPSC|GES|NPSNS1|MCPSC|BCC|NKCHS|BPSCC|RCPSC|NASCC|NCPSC|NPSNS2|ABMGSC|PISE|BNSCC|NASCK|NSTUBBH|CIM|MMA|CPSCL|DPSC|CPSCS|CPSCH|UPIB|BAFSK|CPSCEVJES|BKSP|ClsXI|PROYASHCTG|CCC|BSMRMU|NSTUE|CBBHSC|CBMB|JESC|SAGC|ACC|CBSS)\s.*/i', $userinput, $matches)) {
                                ...
                                elseif (strtoupper($row2['userprocess']) == KEYWORD_UTILITY_INSURANCE_PRIME_LIFE) {
                                    ...
                                    //echo "<br>".strtoupper($row2['userprocess']);
                                    //die();
                                    //echo "opu12<br>";
                                    $output = newCashoutAPI($ussdid, "TRUSTMM ".$row2["userprocess"]." ".$userinput, $carrier);
                                    $output=  $output.'<400';
                                }
                            }
                        }
...
function callTcashAPI($mdn, $text, $operator)
{
    ...
    echo "<br>".$baseUrl . "wcfSMSService.svc/ProcessRequest?msgId=adwwa12&userNumber=$mdn&smstext=$post_text&telcoid=$telcoId&shortCode=16201&encKey=122212";
    die();
