            ...
            while ($row2 = mysqli_fetch_assoc($result2)) {
                ...
                if (in_array($row2["id"], $flashMessageId)) {
                    ...
                    if (in_array($row2["id"], [142, 56])) {
                        ...
                        in_array(strtoupper($skulinputarry[0]), ['CCPC', 'SGC', 'SCBHS', 'BNSCK', 'SHCPSC', 'JCPSC', 'AFMC', 'SCBBHS', 'RGC', 'GCBHS', 'HCPSC', 'RCBS', 'CDC', 'NSTU', 'HSDYKM', 'LIAMAD', 'ARMC', 'AMC', 'MKBMC', 'FCC', 'TIS', 'NCM','CBHSR', 'DC','MIS','RVCBS','NIC' , 'GNIBC', 'IPSC'  , 'BBCPSC','BCPSC','RCES','SRCC','BSISC','CPSCM', 'MESC' ,'SPSCS','HCISC','GTSC','DRMC']) &&
		    ...
		    $sql_input = "SELECT * FROM ussdinputs WHERE msisdn = '".$ussdid."' order by id";
				...
				if (mysqli_num_rows($result2) > 0) {
                                ...
                                if (strtolower($row2["userprocess"]) == 'despr' && $pre_post_data[2] == 2) {
                                    ...
                                    //echo "<br>".$userinput;
                                    //echo "hit API ".getTopUpParams($ussdid, $userinput), $userinput;
				   ...
				   $output=  $output.'<400';*/
                                ...
                                } elseif ($row2["userprocess"] == '' && preg_match('/^\s?(CCPC|SGC|SCBHS|BNSCK|SHCPSC|JCPSC|AFMC|SCBBHS|RGC|GCBHS|HCPSC|RCBS|NSTU|CDC|HSDYKM|LIAMAD|ARMC|AMC|MKBMC|FCC|TIS|NCM|CBHSR|DC|MIS|RVCBS|NIC|GNIBC|IPSC|BBCPSC|BCPSC|RCES|SRCC|BSISC|CPSCM|MESC|SPSCS|HCISC|GTSC|DRMC)\s.*/i', $userinput, $matches)) {
